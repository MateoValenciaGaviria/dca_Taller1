...
public class Logica {
	...
	public Logica(Main app) {
		...
		tubos = new TuboDeMuestras(app, cantidadDeVocales()[0], cantidadDeVocales()[1], cantidadDeVocales()[2],
		...
		for (int i = 0; i < cantPuntos(); i++) {
			bacterias.add(new Bacteria(app));
		}
		pasta = new Pasta(app, cantCombinaciones());
	...
	public void pintar() {
		...
		cadenaCA.pintar();
		cadenaADN.pintar();
		if (app.mouseX > 890 && app.mouseX < 1108 && app.mouseY > 350 && app.mouseY < 515) {
	...
	public void mouse() {
		...
		case PConstants.RIGHT:
			if (app.mouseX > 988 && app.mouseX < 1070 && app.mouseY > 0 && app.mouseY < 240) {
				alarma.pintarSigno();
			}
			break;
		case PConstants.LEFT:
			for (int i = 0; i < bacterias.size(); i++) {
				if(bacterias.get(i).validar(app.mouseX, app.mouseY)){
					bacterias.remove(i);
				}
			}
			break;
	...
	public int encontrarNumero() {
		...
		for (int i = textoCargado.length() - 1; i > 0; i--) {
			if (textoCargado.charAt(i) == '0' || textoCargado.charAt(i) == '1' || textoCargado.charAt(i) == '2'
					|| textoCargado.charAt(i) == '3' || textoCargado.charAt(i) == '4' || textoCargado.charAt(i) == '5'
					|| textoCargado.charAt(i) == '6' || textoCargado.charAt(i) == '7' || textoCargado.charAt(i) == '8'
					|| textoCargado.charAt(i) == '9' && !loEncontro) {
				loEncontro = true;
	...
	public int cantCombinaciones(){
		...
		for (int i = 0; i < textoCargado.length(); i++) {
			if (!Character.isDigit(textoCargado.charAt(i)) && Character.isLetter(textoCargado.charAt(i)) &&
					textoCargado.charAt(i) != 'a' && 
					textoCargado.charAt(i) != 'e' && 
					textoCargado.charAt(i) != 'i' &&
					textoCargado.charAt(i) != 'o' && 
					textoCargado.charAt(i) != 'u') {
					if (i++ < textoCargado.length() &&
						!Character.isDigit(textoCargado.charAt(i)) && 
						 Character.isLetter(textoCargado.charAt(i))) {
					respuesta ++;
				}
			}
		}
	...
	public void agregarCombinaciones(){
		for (int i = 0; i < textoCargado.length(); i++) {
			if (!Character.isDigit(textoCargado.charAt(i)) && Character.isLetter(textoCargado.charAt(i)) &&
					textoCargado.charAt(i) != 'a' && 
					textoCargado.charAt(i) != 'e' && 
					...
					sb.append(textoCargado.charAt(i));
					sb.append(textoCargado.charAt(i++));
					String combinacion = sb.toString();
						pasta.agregarNuevaCombinacion(combinacion);
				}
			}
		}
